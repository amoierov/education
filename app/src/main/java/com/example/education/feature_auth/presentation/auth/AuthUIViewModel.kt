package com.example.education.feature_auth.presentation.auth

import android.util.Patterns
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.example.education.feature_auth.domain.AuthInteract
...
class AuthUIViewModel @Inject constructor(
    ...
    private suspend fun login(email: String, password: String) {
        ...
        delay(1000)
        val result = authInteract.login(email, password)
        if (result.isSuccessful) {
            ...
            if(!authInteract.authenticate()){
                sendAuthEffect(AuthEffect.ShowPopUpError("нету токена"))
            } else {
    ...
    private suspend fun signup(email: String, password: String) {
        ...
        delay(1000)
        val result = authInteract.signup(email, password)
        if (result.isSuccessful) {
            //Чтобы вернуться, удалить позже
            _authState.update {
                AuthStateUI.SignUp(
